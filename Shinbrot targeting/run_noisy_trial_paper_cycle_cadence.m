function trial = run_noisy_trial_paper_cycle_cadence( ...
    trial, sourceState, target, params, noiseControl, noise, ...
    pCurrent, verifiedIdentifier, trialTimer)
%RUN_NOISY_TRIAL_PAPER_CYCLE_CADENCE Cycle-matched paper-mode noisy trial.
%
% Shinbrot et al. state that retargeting was repeated after 40 integration
% steps, "typically about one map cycle", but do not publish the RK4 step
% size.  This reconstruction keeps the present dt and matches the stated
% cadence by retargeting after approximately one mean accepted return.
% Noise is applied only to the propagated plant, never inside deterministic
% forecast searches.

dt = noise.dt;
sigmaNoise = noise.sigmaNoise;
require_cycle_cadence(noise);
...
trial.retargetCadencePolicy = noise.retargetCadencePolicy;
trial.paperReferenceStepCount = noise.paperReferenceStepCount;
trial.retargetPhysicalTime = noise.retargetPhysicalTime;
trial.cadenceRatio = noise.cadenceRatio;
...
for stepIndex = 1:maxRk4Steps
    ...
    if mod(stepIndex, retargetStepInterval) == 0
        ...
        assert_verified_bisection_search( ...
            ...
            'paper cycle-cadence retargeting search', target);
...
if trial.numRetargetingEvents > floor(trial.numRk4Steps / retargetStepInterval)
    error('Cycle-cadence retarget count exceeds the scheduled RK4 cadence.');
end
...
if ~isfield(noise, 'paperReferenceStepCount') || ...
    ...
    error('paperCycleCadenceRetargetMode requires paperReferenceStepCount = 40.');
end
if ~isfield(noise, 'retargetStepInterval') || ...
        ~isfinite(noise.retargetStepInterval) || ...
        noise.retargetStepInterval < 1
    error('paperCycleCadenceRetargetMode requires a finite retargetStepInterval.');
end
if ~isfield(noise, 'cadenceRatio') || ...
        abs(noise.cadenceRatio - 1) > 0.05
    error('paperCycleCadenceRetargetMode requires abs(cadenceRatio - 1) <= 0.05.');
end
end
