    ...
    for j = 1:nB
        for sh = shifts
            ...
            if h.success
                ...
                hits(count).sALo = min(segA.sA(i), segA.sB(i)); %#ok<AGROW>
                hits(count).sAHi = max(segA.sA(i), segA.sB(i)); %#ok<AGROW>
                hits(count).sBLo = min(segB.sA(j), segB.sB(j)); %#ok<AGROW>
                hits(count).sBHi = max(segB.sA(j), segB.sB(j)); %#ok<AGROW>
            end
        end
    end
end
